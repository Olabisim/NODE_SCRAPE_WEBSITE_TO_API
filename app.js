...
const universities_courses = JSON.parse(
  fs.readFileSync(`${__dirname}/data/fetched.json`, "utf-8")
);

let index = 0;
universities_courses.forEach((el) => {
  index++;
  console.log(index, el["university"]);
});
universities_courses.forEach((university_course) => {
  console.log(university_course);
});
